...
def phjSummaryTableToBinaryOutcomes(phjTempDF,
    ...
    try:
        ...
        assert isinstance(phjFailVarName,str), "Parameter 'phjFailStr' needs to be a string."
        ...
        if isinstance(phjVarsToIncludeList,str):
            ...
            for colname in phjVarsToIncludeList:
                assert colname in phjTempDF.columns, "The column name '{0}' does not exist in dataframe.".format(colname)
                
        assert phjSuccVarName in phjTempDF.columns, "The column name '{0}' does not exist in dataframe.".format(phjSuccVarName)
        assert phjFailVarName in phjTempDF.columns, "The column name '{0}' does not exist in dataframe.".format(phjFailVarName)
