...
# Passing collections.Mapping as the phjType parameter to phjAssert() function to check that a variable
# is a dictionary worked OK on Python 3.7 but was ignored when run using Python 3.10. This caused functions
# to return incomplete dataframes. The following webpage provided an explanation and solution:
#   In Python 3.3 "abstract base classes" in collections (like MutableMapping or MutableSequence) were moved
#   to second-level module collections.abc. So in Python 3.3+ the real type is collections.abc.MutableMapping
#   and so on. Documentation states that the old alias names (e.g. collections.MutableMapping) will be available
#   up to Python 3.7 (currently the latest version), however in 3.8 these aliases will be removed.
# See: https://stackoverflow.com/questions/53978542/how-to-use-collections-abc-from-both-python-3-8-and-python-2-7
#
# The following code snippet is based on accepted answer by Freddie Chopin.
# The variable phjCollections is defined and can then be used to pass to phjAssert() function.
# For example, to check that a parameter is a dictionary, rather then using:
# phjAssert('phjMappingDict',phjMappingDict,collections.Mapping)
# which would not work on Python 3.10, use:
# phjAssert('phjMappingDict',phjMappingDict,phjCollections.Mapping)
try:
    phjCollections = collections.abc
except AttributeError:
    phjCollections = collections
...
def phjReverseMap(phjDF,
    ...
    try:
        ...
        # To check that a variable is a dictionary, pass phjCollections.Mapping as
        # phjType parameter to phjAssert() function. See top of file for explanation.
        phjAssert('phjMappingDict',phjMappingDict,phjCollections.Mapping)
